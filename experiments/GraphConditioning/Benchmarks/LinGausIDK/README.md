# LinGausIDK Benchmark - Partial Graph Knowledge

This benchmark extends the LinGaus benchmark to handle **partial graph knowledge** scenarios, where the causal structure is only partially known. This simulates realistic situations where some edges are confirmed, some are known to be absent, and others are uncertain.

## Overview

The LinGausIDK benchmark generates Linear-Gaussian Structural Causal Models (SCMs) with adjacency matrices in a **three-state format** `{-1, 0, 1}`:
...
├── LingausBenchmark.py          # Benchmark runner class
...
benchmark = LinGausBenchmark(
    benchmark_dir="/path/to/LinGausIDK",
    verbose=True,
...
## Differences from Original LinGaus
