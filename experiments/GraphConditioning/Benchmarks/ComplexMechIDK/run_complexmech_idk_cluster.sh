...
# Unzip ComplexMechIDK benchmark data
echo "Unzipping ComplexMechIDK benchmark data..."
if [ -f "data_cache_ComplexMechIDK.zip" ]; then
	unzip -o -q data_cache_ComplexMechIDK.zip
	...
	echo "ComplexMechIDK benchmark data unzipped. Contents:"
	ls -la data_cache | head -20 || true
else
	echo "ERROR: No data_cache_ComplexMechIDK.zip found; cannot run benchmark"
	exit 1
fi

# Set benchmark directory
export COMPLEXMECH_IDK_BENCHMARK_DIR="${PWD}"
echo "Exported COMPLEXMECH_IDK_BENCHMARK_DIR=${COMPLEXMECH_IDK_BENCHMARK_DIR}"
