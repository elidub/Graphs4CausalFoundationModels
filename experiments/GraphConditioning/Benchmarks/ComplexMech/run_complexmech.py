#!/usr/bin/env python3
"""
Standalone ComplexMech Benchmark Runner.

This script runs the ComplexMech benchmark on a trained model checkpoint.
It can be used independently or as part of a cluster job submission.

Usage:
    python run_complexmech.py --config /path/to/config.yaml --checkpoint /path/to/checkpoint.pt --fidelity high
...
def main():
    parser = argparse.ArgumentParser(
        description="Run ComplexMech benchmark on a trained model checkpoint.",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory to save results (default: checkpoint directory/complexmech_<fidelity>)"
    )
    parser.add_argument(
        "--benchmark_dir",
        type=str,
        default=None,
        help="Directory containing ComplexMech configs and data_cache (default: script directory)"
    )
    parser.add_argument(
        "--use_ancestor_matrix",
        action="store_true",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Specific node counts to evaluate (default: all [2, 5, 20, 50])"
    )
    parser.add_argument(
        "--variants",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Specific sample sizes for path variants (default: all [500, 700, 800, 900, 950])"
    )
    
    args = parser.parse_args()
    
    print("=" * 80)
    print("ComplexMech Benchmark Runner")
    print("=" * 80)
    ...
    if args.benchmark_dir:
        ...
        if env_benchmark_dir:
            ...
            print(f"Using COMPLEXMECH_BENCHMARK_DIR from environment: {benchmark_dir}")
        else:
    ...
    if args.output_dir:
        ...
        # Default to checkpoint directory / complexmech_<fidelity>
        output_dir = checkpoint_path.parent / f"complexmech_{args.fidelity}"
    ...
    try:
        ...
        print("Initializing ComplexMech benchmark...")
        ...
        print(f"  Available sample sizes: {benchmark.SAMPLE_SIZES}")
        print()
        ...
        if not data_cache.exists():
            ...
            print(f"Please run sample_data.py first to generate benchmark data.")
            sys.exit(1)
        ...
        for key, value in results.items():
            if isinstance(key, tuple):
                ...
                if sample_size is None:
                    ...
                    str_key = f"{node_count}node_{variant}_ntest{sample_size}"
                results_serializable[str_key] = value
            else:
                results_serializable[str(key)] = value
        
        # Save JSON
        results_json_path = output_dir / f"complexmech_benchmark_{args.fidelity}.json"
        ...
        try:
            ...
            if rows:
                ...
                results_csv_path = output_dir / f"complexmech_benchmark_{args.fidelity}.csv"
        ...
        for variant, results_list in sorted(variants_summary.items()):
            print(f"\nVariant: {variant}")
            print("-" * 40)
            for r in sorted(results_list, key=lambda x: (x['node_count'], x['sample_size'] or 0)):
                sample_str = f"ntest={r['sample_size']}" if r['sample_size'] else "base"
                print(f"  {r['node_count']:2d} nodes, {sample_str:12s}: MSE={r['mse_mean']:.6f}, R²={r['r2_mean']:.4f}")
