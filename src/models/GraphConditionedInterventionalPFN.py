...
class GraphConditionedInterventionalPFN(nn.Module):
    ...
    Adjacency matrix format (NEW - updated to match dataset):
    - Shape: (B, L+2, L+2) where L is number of features (after dropout)
    - Position 0: Treatment variable (intervention_node)
    - Position 1: Outcome variable (target feature)
    - Positions 2 to L+1: Feature variables (kept after dropout, sorted order, correspond to X[:,0] to X[:,L-1])
...
if __name__ == "__main__":
    ...
    # Adjacency matrix ordering: [T, Y, X_0, X_1, ..., X_{L-1}]
    # Position 0: Treatment (T)
    # Position 1: Outcome (Y)
    # Positions 2 to L+1: Features (X_0 to X_{L-1})
    # Note: Self-loops are added automatically by the model
    sparse_adj = torch.zeros(B, L + 2, L + 2)
    # Treatment (position 0) affects outcome (position 1)
    sparse_adj[:, 1, 0] = 1
    # Treatment affects all features (positions 2 to L+1)
    sparse_adj[:, 2:, 0] = 1
    # Chain structure: each feature affects the next
    for i in range(2, L + 1):
        sparse_adj[:, i + 1, i] = 1 if i + 1 < L + 2 else 0
