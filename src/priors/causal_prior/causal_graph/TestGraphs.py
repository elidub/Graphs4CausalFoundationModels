...
class TestGraphSamplerAndCausalGraph(unittest.TestCase):
    """
    Class to test graph sampling and causal dag itselfs
    """
    def setUp(self) -> None:
        self.n = 25
        self.p = 0.2
    ...
    # ---------- GraphSampler: vectorized ----------
    def test_vectorized_basic(self):
        G, perm = self.sampler.sample_ER_DAG(self.n, self.p, return_perm=True)
    ...
    def test_legacy_basic(self):
        G, perm = self.sampler.sample_ER_DAG_legacy(self.n, self.p, return_perm=True)
        self.assertEqual(G.number_of_nodes(), self.n)
        self.assertTrue(nx.is_directed_acyclic_graph(G))
        pos = {node: i for i, node in enumerate(perm)}
        for u, v in G.edges():
    ...
    def test_legacy_extremes(self):
        G0 = self.sampler.sample_ER_DAG_legacy(self.n, 0.0)
        self.assertEqual(G0.number_of_edges(), 0)
        self.assertTrue(nx.is_directed_acyclic_graph(G0))

        G1 = self.sampler.sample_ER_DAG_legacy(self.n, 1.0)
        self.assertEqual(G1.number_of_edges(), self.n * (self.n - 1) // 2)
        self.assertTrue(nx.is_directed_acyclic_graph(G1))

    # ---------- Reproducibility ----------
    def test_reproducibility_vectorized(self):
        G1, perm1 = self.sampler.sample_ER_DAG(self.n, self.p, return_perm=True)
        sampler2 = GraphSampler(seed=self.seed)
        G2, perm2 = sampler2.sample_ER_DAG(self.n, self.p, return_perm=True)

        self.assertTrue(np.array_equal(perm1, perm2))
        self.assertEqual(set(G1.edges()), set(G2.edges()))

    def test_reproducibility_legacy(self):
        G1, perm1 = self.sampler.sample_ER_DAG_legacy(self.n, self.p, return_perm=True)
        sampler2 = GraphSampler(seed=self.seed)
        G2, perm2 = sampler2.sample_ER_DAG_legacy(self.n, self.p, return_perm=True)
    ...
    def test_integration_with_causalgraph(self):
        G, _ = self.sampler.sample_ER_DAG(self.n, self.p, return_perm=True)
    ...
    def test_mutations_and_cache_invalidation(self):
        G, _ = self.sampler.sample_ER_DAG(8, 0.3, return_perm=True)
        cg = CausalDAG(g=G, check_acyclic=True)
    ...
    def test_invalid_inputs_sampler(self):
        with self.assertRaises(ValueError):
            self.sampler.sample_ER_DAG(-1, 0.2)
        with self.assertRaises(ValueError):
            self.sampler.sample_ER_DAG(5, -0.1)
        with self.assertRaises(ValueError):
            self.sampler.sample_ER_DAG(5, 1.1)

        with self.assertRaises(ValueError):
            self.sampler.sample_dag(5, 0.2, method="unknown")  # type: ignore
