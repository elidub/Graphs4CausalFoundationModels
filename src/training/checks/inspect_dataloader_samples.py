...
class DataloaderDatasetVisualizer:
    ...
    def __init__(self, config_path: str, seed: int = 42):
        ...
        print("[INFO] Initializing DataloaderDatasetVisualizer")
        print(f"[INFO] Config path: {self.config_path}")
        print(f"[INFO] Seed: {self.seed}")
    ...
    def load_config(self):
        ...
        print(f"[INFO] Experiment: {self.config.get('experiment_name', 'unnamed')}")
        print(f"[INFO] Description: {self.config.get('description', 'no description')}")
    ...
    def create_dataset_and_dataloader(self, n_datasets_per_batch=None):
        ...
        print(f"[INFO] SCM nodes: {scm_config.get('num_nodes', {}).get('value', 'unknown')}")
        print(f"[INFO] Dataset size: {dataset_config.get('dataset_size', {}).get('value', 'unknown')}")
        print(f"[INFO] Max features: {dataset_config.get('max_number_features', {}).get('value', 'unknown')}")
        ...
        if n_datasets_per_batch is not None:
            ...
            print(f"[INFO] Overriding batch_size with N_DATASETS_PER_BATCH: {batch_size}")
        else:
    ...
    def extract_tabular_data_from_batch(self, batch):
        ...
        if isinstance(batch, (list, tuple)) and len(batch) == 4:
            ...
            print(f"[INFO] SimplePFN format detected:")
            print(f"[INFO]   Batch size: {batch_size}")
            ...
            print(f"[WARN] Unexpected batch format: {type(batch)}")
            if hasattr(batch, '__len__'):
                print(f"[WARN] Batch length: {len(batch)}")
    ...
    def visualize_dataset(self, X_train, y_train, X_test, y_test, dataset_idx, batch_idx, save_dir=None):
        ...
        print(f"[INFO] Visualizing dataset {dataset_idx} from batch {batch_idx}")
        ...
        print(f"[INFO] Original shapes from dataloader (with padding):")
        print(f"[INFO]   X_train: {X_train.shape} | X_test: {X_test.shape}")
        print(f"[INFO]   y_train: {y_train.shape} | y_test: {y_test.shape}")
        print(f"[INFO]   Total samples: Train={n_train_samples}, Test={n_test_samples}")
        print(f"[INFO]   Total features: {n_features}")
        
        # Config values for comparison
        print(f"[INFO] Config values:")
        print(f"[INFO]   max_number_samples: 100 (from early_test.yaml)")
        print(f"[INFO]   max_number_features: 10 (from early_test.yaml)")
        print(f"[INFO]   number_samples_per_dataset: uniform(50, 99)")
        
        # Data ranges
        print(f"[INFO] Data ranges:")
        print(f"[INFO]   X_train: [{X_train.min():.3f}, {X_train.max():.3f}]")
        print(f"[INFO]   y_train: [{y_train.min():.3f}, {y_train.max():.3f}]")
        print(f"[INFO]   X_test: [{X_test.min():.3f}, {X_test.max():.3f}]")
        print(f"[INFO]   y_test: [{y_test.min():.3f}, {y_test.max():.3f}]")
        ...
        print(f"[INFO]   Total features (with padding): {n_features}")
        print(f"[INFO]   Active features (actual data): {n_active_features}")
        print(f"[INFO]   Padded features (zeros): {n_padded_features}")
        print(f"[INFO]   Feature padding fraction: {feature_padding_fraction:.1%}")
        
        print(f"[INFO] TRAIN SAMPLES:")
        print(f"[INFO]   Total train samples (with padding): {n_train_samples}")
        print(f"[INFO]   Active train samples (actual data): {n_active_train_samples}")
        print(f"[INFO]   Padded train samples (zeros): {n_padded_train_samples}")
        print(f"[INFO]   Train sample padding fraction: {train_sample_padding_fraction:.1%}")
        
        print(f"[INFO] TEST SAMPLES:")
        print(f"[INFO]   Total test samples (with padding): {n_test_samples}")
        print(f"[INFO]   Active test samples (actual data): {n_active_test_samples}")
        print(f"[INFO]   Padded test samples (zeros): {n_padded_test_samples}")
        print(f"[INFO]   Test sample padding fraction: {test_sample_padding_fraction:.1%}")
        
        # Original vs Config comparison
        total_original_samples = n_active_train_samples + n_active_test_samples
        print("="*80)
        print("                  CONFIG vs ACTUAL COMPARISON")
        print("="*80)
        print(f"[INFO] SAMPLE COUNT ANALYSIS:")
        print(f"[INFO]   Config max_number_samples: 100")
        print(f"[INFO]   Config expected range: 50-99 samples")
        print(f"[INFO]   Actual total samples: {total_original_samples}")
        print(f"[INFO]   Actual train samples: {n_active_train_samples}")
        print(f"[INFO]   Actual test samples: {n_active_test_samples}")
        
        print(f"[INFO] FEATURE COUNT ANALYSIS:")
        print(f"[INFO]   Config max_number_features: 10")
        print(f"[INFO]   Actual active features: {n_active_features}")
        print(f"[INFO]   Features lost to preprocessing: {10 - n_active_features}")
        ...
        print(f"[INFO] PREPROCESSING IMPACT ANALYSIS:")
        print(f"[INFO]   Expected feature loss sources:")
        print(f"[INFO]     - Target feature removed: 1 feature")
        print(f"[INFO]     - Feature dropout (config 0.1): ~{int(0.1 * 10)} features expected")
        print(f"[INFO]     - Original SCM features: 3-8 (from scm_config.num_nodes)")
        expected_remaining = 10 - 1  # Target removed
        expected_after_dropout = expected_remaining * (1 - 0.1)  # 10% dropout
        print(f"[INFO]     - Expected remaining after target removal: {expected_remaining}")
        print(f"[INFO]     - Expected after 10% dropout: ~{expected_after_dropout:.1f}")
        print(f"[INFO]     - Actual remaining: {n_active_features}")
        
        # CRITICAL DEBUGGING: The shape inconsistency!
        print(f"[CRITICAL] SHAPE INCONSISTENCY DETECTED:")
        print(f"[CRITICAL]   Dataloader reported 9 features, config expects 10")
        print(f"[CRITICAL]   This suggests preprocessing already removed features BEFORE padding")
        print(f"[CRITICAL]   Likely causes:")
        print(f"[CRITICAL]     1. Original SCM generated < 10 features")
        print(f"[CRITICAL]     2. Feature dropout happened before padding")
        print(f"[CRITICAL]     3. Target feature was already removed")
        ...
        print(f"[INFO] FEATURE PATTERN ANALYSIS:")
        print(f"[INFO]   Active feature positions: {active_indices.tolist()}")
        print(f"[INFO]   Zeroed feature positions: {zeroed_indices.tolist()}")
        if len(zeroed_indices) > 0:
            if np.all(zeroed_indices >= n_active_features):
                print(f"[INFO]   Pattern: All zeros at end → Likely padding-only")
                print(f"[CRITICAL]   CONCLUSION: Original data had {n_active_features} features")
                print(f"[CRITICAL]   Missing features were likely removed in BasicProcessing, not padding")
            elif np.all(zeroed_indices < n_active_features):
                print(f"[INFO]   Pattern: Zeros at beginning → Likely preprocessing dropout")
            else:
                print(f"[INFO]   Pattern: Mixed zeros → Combination of dropout + padding")
                
        # Additional analysis for test sample padding
        print(f"[CRITICAL] TEST SAMPLE PADDING ANALYSIS:")
        print(f"[CRITICAL]   Test samples show {test_sample_padding_fraction:.1%} padding ({n_padded_test_samples}/{n_test_samples} are zeros)")
        print(f"[CRITICAL]   This suggests the original dataset had {n_active_test_samples} test samples")
        print(f"[CRITICAL]   Expected from config: 30% of total samples should be test")
        print(f"[CRITICAL]   With {total_original_samples} total samples: 0.3 * {total_original_samples} = {int(0.3 * total_original_samples)} expected test samples")
        print(f"[CRITICAL]   But we have {n_active_test_samples} actual test samples!")
        if n_active_test_samples < int(0.3 * total_original_samples):
            print(f"[CRITICAL]   This indicates train_fraction = 0.7 was applied to a smaller dataset")
        else:
            print(f"[INFO]   Test sample count is reasonable for the dataset size")
        
        # Detailed feature analysis
        print("="*80)
        print("                    DETAILED FEATURE ANALYSIS")
        print("="*80)
        print(f"[DEBUG] Feature variances - Train: {train_feature_variances}")
        print(f"[DEBUG] Feature variances - Test: {test_feature_variances}")
        print(f"[DEBUG] Active features mask: {active_features_mask}")
        print(f"[DEBUG] Active feature indices: {np.where(active_features_mask)[0].tolist()}")
        print(f"[DEBUG] Zeroed feature indices: {np.where(~active_features_mask)[0].tolist()}")
        
        # Detailed sample analysis (show first/last few)
        print(f"[DEBUG] Active train samples: first 10 = {active_train_samples_mask[:10]}")
        print(f"[DEBUG] Active train samples: last 10 = {active_train_samples_mask[-10:]}")
        print(f"[DEBUG] Active test samples: first 10 = {active_test_samples_mask[:10]}")
        print(f"[DEBUG] Active test samples: last 10 = {active_test_samples_mask[-10:]}")
        
        print("="*80)
        
        if n_active_features == 0:
            print(f"[WARN] No active features for dataset {dataset_idx}, skipping visualization")
            return
        
        if n_active_train_samples == 0 or n_active_test_samples == 0:
            print(f"[WARN] No active samples for dataset {dataset_idx}, skipping visualization")
            return
        ...
        print(f"[INFO] After removing padding:")
        print(f"[INFO]   Filtered train: {X_train_filtered.shape[0]} samples × {X_train_filtered.shape[1]} features")
        print(f"[INFO]   Filtered test: {X_test_filtered.shape[0]} samples × {X_test_filtered.shape[1]} features")
        print(f"[INFO]   X_train_filtered range: [{X_train_filtered.min():.3f}, {X_train_filtered.max():.3f}]")
        print(f"[INFO]   X_test_filtered range: [{X_test_filtered.min():.3f}, {X_test_filtered.max():.3f}]")
        ...
        print("[INFO] Analyzing target predictability (train->test)...")
        ...
        print("[INFO] Creating feature scatterplot matrix...")
        ...
        print(f"[INFO] Plotting {max_features_for_plots} features")
        ...
        if save_dir:
            ...
            print(f"[OK] Saved scatterplot matrix to: {save_path}")
        ...
        if n_active_features > 1:
            ...
            print("[INFO]   Feature correlations with target:")
            target_corrs = corr_matrix['target'][:-1]  # Exclude target-target correlation
            for feat, corr in target_corrs.items():
                print(f"[INFO]     {feat}: {corr:.3f}")
        else:
            print("[INFO] Only 1 active feature, skipping correlation heatmap")
            
            # Simple statistics for single feature
            print("[INFO] Dataset summary statistics:")
            print(f"[INFO]   Single feature: {active_feature_names[0]}")
            print(f"[INFO]   Feature range: [{X_train_filtered.min():.3f}, {X_train_filtered.max():.3f}]")
            print(f"[INFO]   Feature std: {X_train_filtered.std():.3f}")
            correlation_with_target = np.corrcoef(X_train_filtered.flatten(), y_train_filtered)[0, 1]
            print(f"[INFO]   Correlation with target: {correlation_with_target:.3f}")
    ...
    def _analyze_predictability(self, X, y, feature_names):
        ...
        try:
            ...
            if n_samples < 10 or n_features == 0:
                print("[INFO] Too few samples/features for predictability analysis")
                return
            
            print(f"[INFO] Predictability analysis ({n_samples} samples, {n_features} features):")
            ...
            try:
                ...
                if n_samples >= 20:
                    ...
                    print(f"[INFO]   Linear Model R²: {linear_r2:.3f} (train), {linear_cv_mean:.3f}±{linear_cv_std:.3f} (CV)")
                else:
                    print(f"[INFO]   Linear Model R²: {linear_r2:.3f} (train only)")
                    
            except Exception as e:
                print(f"[WARN]   Linear model failed: {e}")
            ...
            try:
                ...
                if n_samples >= 20:
                    ...
                    print(f"[INFO]   Random Forest R²: {rf_r2:.3f} (train), {rf_cv_mean:.3f}±{rf_cv_std:.3f} (CV)")
                    
                    # Feature importance
                    if n_features <= 10:  # Only show for reasonable number of features
                        importances = rf_model.feature_importances_
                        print("[INFO]   Feature Importance (Random Forest):")
                        for feat, imp in zip(feature_names, importances):
                            print(f"[INFO]     {feat}: {imp:.3f}")
                else:
                    print(f"[INFO]   Random Forest R²: {rf_r2:.3f} (train only)")
                    
            except Exception as e:
                print(f"[WARN]   Random Forest failed: {e}")
                
        except ImportError:
            print("[WARN] sklearn not available for predictability analysis")
        except Exception as e:
            print(f"[WARN] Predictability analysis failed: {e}")
    ...
    def _analyze_predictability_train_test(self, X_train, y_train, X_test, y_test, feature_names):
        ...
        try:
            ...
            if n_train < 5 or n_test < 5 or n_features == 0:
                print("[INFO] Too few samples/features for train-test analysis")
                return
            
            if n_features == 1:
                print("[INFO] Single feature analysis:")
                print("[INFO]   Note: Limited predictive analysis possible with only one feature")
            
            print(f"[INFO] Train-Test predictability analysis ({n_train} train, {n_test} test, {n_features} features):")
            ...
            try:
                ...
                print(f"[DEBUG] Linear Model - Train predictions (first 5): {train_pred[:5]}")
                print(f"[DEBUG] Linear Model - Train actual (first 5): {y_train[:5]}")
                print(f"[DEBUG] Linear Model - Test predictions (first 5): {test_pred[:5]}")
                print(f"[DEBUG] Linear Model - Test actual (first 5): {y_test[:5]}")
                
                print("[INFO]   Linear Model:")
                print(f"[INFO]     Train R²: {train_r2:.6f}, MSE: {train_mse:.8f}")
                print(f"[INFO]     Test R²: {test_r2:.6f}, MSE: {test_mse:.8f}")
                
                # Check for suspicious results
                if test_mse < 1e-6:
                    print(f"[WARN]     Suspiciously low test MSE: {test_mse:.2e}")
                if test_r2 < -10:
                    print(f"[WARN]     Extremely poor test R² (worse than baseline): {test_r2:.6f}")
                elif test_r2 > 0.99:
                    print(f"[WARN]     Suspiciously high test R² (possible overfitting): {test_r2:.6f}")
            ...
            print("[WARN] sklearn not available for train-test analysis")
        except Exception as e:
            print(f"[WARN] Train-test analysis failed: {e}")
    
    
    def sample_and_visualize_batches(self, n_batches=2, n_datasets_per_batch=3, save_dir=None):
        """
        ...
        print(f"[INFO] Target: {n_batches} batches, {n_datasets_per_batch} datasets per batch")
        ...
        for batch_idx, batch in enumerate(self.dataloader):
            ...
            if not datasets:
                print(f"[WARN] No datasets extracted from batch {batch_idx}")
                continue
            
            print(f"[OK] Extracted {len(datasets)} datasets from batch")
            
            # Visualize specified number of datasets
            n_to_visualize = min(n_datasets_per_batch, len(datasets))
            print(f"[INFO] Visualizing first {n_to_visualize} datasets...")
    ...
    def run_visualization(self, n_batches=2, n_datasets_per_batch=3, save_dir=None):
        """Run complete visualization: config -> dataloader -> batch sampling -> visualization."""
        print("=" * 60)
        print("      DataLoader Dataset Visualization from YAML Config")
        print("=" * 60)
        ...
        try:
            ...
            print("=" * 60)
            print("[OK] Complete visualization finished successfully!")
            print("=" * 60)
...
def main():
    ...
    N_DATASETS_PER_BATCH = 1
    
    # Directory to save plots (None = don't save, just display)
    SAVE_DIR = None
