...
def dofm_pipeline(model, cate_dataset):
    ...
    # T->Y and X->T MODE: Only T -> Y and X -> T relationships are known
    # - T -> Y: known (1)
    # - X -> T: known (1) for real features
    # - X -> Y: unknown (0)
    # - X -> X: unknown (0)
    
    # Set the known T -> Y edge
    adjacency_matrix[T_idx, Y_idx] = 1.0
    
    # Set the known X -> T edges for real features
    for i in range(n_real_features):
        adjacency_matrix[i, T_idx] = 1.0
    
    # X -> Y remains unknown (0)
    
    # Only PADDED features get -1 (no edge) since they don't exist
...
if __name__ == "__main__":
    ...
    checkpoint_path = "/Users/arikreuter/Documents/PhD/CausalPriorFitting/experiments/FirstTests/checkpoints/final_earlytest_5node_16773270.0/final.pt"
    model_config_path = "/Users/arikreuter/Documents/PhD/CausalPriorFitting/experiments/FirstTests/checkpoints/final_earlytest_5node_16773270.0/final_config.yaml"
