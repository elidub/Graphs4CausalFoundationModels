...
MODEL_CONFIG = None
# Global variable for test feature masking mode (set via command line)
MASK_TEST_FEATURES = False
...
def dofm_variable_conditioning_pipeline(model, cate_dataset):
    """
    Use the GraphConditionedInterventionalPFN model (variable conditioning variant) to predict CATE.
    
    This model was trained with variable test_feature_mask_fraction from Uniform(0,1),
    meaning it can handle both:
    - Full conditioning: test features visible (MASK_TEST_FEATURES=False)
    - No conditioning: test features hidden/zeroed (MASK_TEST_FEATURES=True)
    ...
    """
    global MODEL_CONFIG, MASK_TEST_FEATURES
    ...
    # Report test feature masking mode
    if MASK_TEST_FEATURES:
        print(f"Variable conditioning mode: test features MASKED (zeroed out)")
    else:
        print(f"Variable conditioning mode: test features VISIBLE (full conditioning)")
    ...
    # Apply test feature masking if enabled
    if MASK_TEST_FEATURES:
        # Zero out all test features (100% masking)
        X_test = np.zeros_like(X_test)
        print(f"Applied test feature masking: all test features zeroed")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run DOFM variable conditioning experiments.")
    ...
    parser.add_argument("--exp_name", type=str, required=True, help="Experiment name for results")
    parser.add_argument("--mask_test_features", action="store_true", 
                        help="If set, zero out all test features (no conditioning). If not set, test features are visible (full conditioning).")

    args = parser.parse_args()
    
    # Set global masking mode
    ...
    print(f"Test feature mode: {mode_str}")
    ...
    for dataset in datasets_to_run:
        print(f"\n{'='*60}")
        print(f"--- Starting Variable Conditioning Experiment ---")
        print(f"Dataset: {dataset}")
        print(f"Model:   {args.model}")
        print(f"Test features: {mode_str}")
        print(f"{'='*60}\n")
        ...
        try:
            evaluate_pipeline(
                ...
                model_pipeline=dofm_variable_conditioning_pipeline,
                model=model,
                args=dataset_args)
