"""
T-Learner variant for PSID imbalance: use all treated samples and up to 500 controls.

Training: all T=1 samples + random 500 T=0 samples (if available).
Testing: use full test set.

For T-learner:
- Treated model: trains on all T=1 samples
- Control model: trains on up to 500 T=0 samples
"""
...
def tlearner_psid_balanced_pipeline(model, cate_dataset):
    """
    T-learner pipeline that for PSID uses all treated samples and up to 500 controls.
    For other datasets this behaves like the regular T-learner.
    
    T-learner approach:
    1. Train two separate models: one on treated units (T=1), one on control units (T=0)
    2. For PSID: use all treated, subsample controls to max 500
    3. Predict outcomes for test set using both models
    4. CATE = E[Y|T=1,X] - E[Y|T=0,X]
    """
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run T-learner PSID-balanced experiment")
    ...
    evaluate_pipeline(
        ...
        model_pipeline=tlearner_psid_balanced_pipeline,
        model=model,
        args=args
    )

    print("T-learner PSID-balanced finished")
