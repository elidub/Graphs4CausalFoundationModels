...
def tlearner_pipeline(model, cate_dataset):
    """
    Use SimplePFN as a T-learner to predict CATE.
    
    T-learner approach:
    1. Train two separate models: one on treated units (T=1), one on control units (T=0)
    2. Predict outcomes for test set using both models
    ...
    # SimplePFN expects num_features + 1 (features + target)
    ...
    # Scale targets to [-1, 1] for both groups
    ...
    print(f"Scaled target ranges - Treated: [{np.min(y_train_treated_scaled):.4f}, {np.max(y_train_treated_scaled):.4f}], "
          f"Control: [{np.min(y_train_control_scaled):.4f}, {np.max(y_train_control_scaled):.4f}]")
    
    # Predict Y for T=1 using treated model
    # SimplePFN.predict expects (X_train, y_train, X_test)
    y_pred_1_scaled = model.predict(
        X_train=X_train_treated,
        y_train=y_train_treated_scaled,  # Use SCALED targets
        X_test=X_test,
        prediction_type="mean",  # Use mean prediction
        aggregate="mean"  # If ensemble, aggregate with mean
    )
    
    # Predict Y for T=0 using control model
    y_pred_0_scaled = model.predict(
        X_train=X_train_control,
        y_train=y_train_control_scaled,  # Use SCALED targets
        X_test=X_test,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run T-learner baseline experiments.")
    ...
    simple_pfn = SimplePFNSklearn(
        ...
        verbose=True,
        n_estimators=1,  # Single model (not ensemble) for T-learner
    ...
    for dataset in datasets_to_run:
        print(f"\n{'='*60}")
        print(f"--- Starting T-Learner Experiment ---")
        ...
        try:
            evaluate_pipeline(
                ...
                model_pipeline=tlearner_pipeline,
                model=model,
                args=dataset_args)
            print(f"\n--- {dataset} T-Learner Experiment Finished ---")
        except Exception as e:
            print(f"\n--- {dataset} T-Learner Experiment FAILED: {e} ---")
            import traceback
            traceback.print_exc()
            continue
    
    print(f"\n{'='*60}")
    print(f"--- All T-Learner Experiments Finished ---")
    print(f"{'='*60}")    
