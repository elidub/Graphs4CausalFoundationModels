...
def slearner_pipeline(model, cate_dataset):
    ...
    # Create test data with T=1 (treated) and T=0 (control) using target-encoded values
    X_test_with_T1 = np.hstack([X_test, np.full((n_test, 1), t_intv_1_encoded)])   # T=1 (target encoded)
    X_test_with_T0 = np.hstack([X_test, np.full((n_test, 1), t_intv_0_encoded)])   # T=0 (target encoded)
